...
| `review.html` | 카드사 제출용 단일 심사 URL. 일반결제와 정기결제 상품·조건·동의·테스트 결제창을 한 페이지에서 제공한다 |
| `order.html` | 심사용 실물책 주문 페이지. PortOne V2 SDK로 KG이니시스 테스트 결제창을 연다. 내비게이션에 노출하지 않고 URL로만 전달한다 |
| `subscription.html` | 심사용 월 구독 카드 등록 페이지. 별도의 KG이니시스 정기결제 테스트 채널로 빌링키 발급창을 열며 실제 요금은 청구하지 않는다 |
...
`policies/manifest.json`은 문서별 `termsCode`, version, locale, 공고일, 시행일, SHA-256, 원본 파일과 공개 경로를 연결한다. `policies/SHA256SUMS`는 manifest와 같은 해시를 담으며 검증 과정에서 서로 일치해야 한다. 무버전 경로(`aliases`)는 현재 시행 중인 version의 원문 사본을 제공하며, 시행일이 아직 오지 않은 version으로는 옮기지 않는다.
...
일반결제·정기결제 심사 화면의 필수 고지, 동의, 사업자 정보와 운영 시크릿 미포함 여부는 다음 명령으로 확인한다.

```bash
./scripts/check-payment-review-pages.sh
```
...
- 구독 플랜은 Basic 월 9,900원(동화 4권), Premium 월 19,900원(동화 12권)으로 심사 화면과 정책에 동일하게 표시한다.
...
1. 노란 표시(`mark.todo`)가 남아 있지 않은지 확인한다.
...
3. `order.html`은 일반결제 테스트 채널(`INIpayTest`), `subscription.html`은 정기결제 테스트 채널(`INIBillTst`)을 각각 사용한다. 운영 MID나 비밀키는 정적 파일에 넣지 않는다. 운영 전환은 서버 환경설정과 서버 검증·빌링키 보관을 갖춘 뒤 별도로 진행한다.
4. 통신판매업신고번호는 신고증이 나온 뒤 "신고 진행 중"을 번호로 바꾼다(index, order, subscription 푸터).
5. 현재 고객센터에는 실제 연락 가능한 휴대전화 번호를 표시한다. PG·카드사가 유선 대표번호를 요구하면 인터넷전화 또는 대표번호를 개통한 뒤 모든 페이지의 연락처를 함께 바꾼다. 임의 번호를 사용하지 않는다.
6. 약관·개인정보처리방침·환불정책은 초안이다. 공개 전에 법률 검토를 받는다.
...
| 구매 시 이니시스 결제창 노출 | `review.html` #general-payment |
| 정기결제 카드 등록창·가격·결제주기·필수 동의 | `review.html` #recurring-payment |
| 이용약관 | `terms.html` |
