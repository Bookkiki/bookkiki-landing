...
내부 체크리스트다. 이미 발행된 version의 원문은 변경하지 않는다. 미확정 값이 확정되고
법률 검토가 끝나기 전에는 새 개인정보처리방침 version을 발행하지 않는다.

**현황(2026-09-18):** 아래 "공개 정책 발행 전 확인" 표의 값은 개인정보처리방침
[2026-09-17-1](../ko-KR/policies/privacy-policy/2026-09-17-1/index.html) 판으로 발행됐다
(Google LLC의 Google Analytics for Firebase 처리위탁, 미국을 포함한 Google 데이터센터
처리·보관 고지, 이용자·이벤트 단위 데이터 보관기간 최대 14개월, 회사가 직접 기록하는
이벤트 이름 27종). 앱은 설정 외에 가입 약관 화면에서도 앱 사용 분석 동의(선택)를 받는다
(bookkiki-client #367). 콘솔·스토어 체크리스트의 미완료 항목은 그대로 남아 있다.

## 확정된 제품 원칙

- Firebase Analytics 수집 기본값은 OFF다.
- 보호자가 가입 약관 화면 또는 설정에서 별도로 동의한 뒤에만 수집한다.
...
| 처리 항목 | 발행됨(2026-09-17-1) | 위 allowlist 이벤트 27종과 Firebase 기본 이벤트·기기/앱 기본 정보·대략적 지역 |
| 법적 근거 | 구현 확정 | 보호자 선택 동의 전 수집 중지 |
| 처리자·수탁자 | 발행됨(2026-09-17-1) | Google LLC(Google Analytics for Firebase) |
| 국외 이전 국가·시점·방법 | 발행됨(2026-09-17-1) | 미국을 포함한 Google 데이터센터, 동의 후 수집 시점부터 네트워크 전송, 개인정보 보호법 제28조의8 제1항 제3호 고지 |
| 보유 기간 | 발행됨(2026-09-17-1) | 이용자·이벤트 단위 데이터 최대 14개월. 콘솔 data retention 값의 화면 증거 보존은 아래 체크리스트에 남아 있다 |
| 파기·철회 효과 | 발행됨(2026-09-17-1) | 철회 시 새 수집 중단·기기 분석 식별자 초기화, 집계 통계는 남을 수 있음을 고지 |
| 문의·권리 행사 | 발행됨(2026-09-17-1) | 개인정보처리방침의 담당자·연락처 절을 그대로 사용 |
